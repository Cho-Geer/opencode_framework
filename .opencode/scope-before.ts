...
try { appendFileSync(".task_temp/_diag-scope-before.txt", "LOADED\n"); } catch {}
...
const DIAG = (msg: string) => {
  try {
    appendFileSync(
      ".task_temp/_diag-scope-before.txt",
...
async function toolExecuteBefore(input: any, output: any): Promise<void> {
  try {
    ...
    DIAG(
      `resolveAgent(sid=${String(input.sessionID).slice(-12)}) → "${agent}" (${typeof agent}) | tool=${input.tool}`,
    );
    ...
    // FW-KNOWLEDGE-GATE: strict/locked — non-SA must attest knowledge cache before modify tools
    const isSA = agent === "@Super-Admin" || agent === "Super-Admin";
    if ((mode === "strict" || mode === "locked") && !isSA) {
      const kcs = readSubState("knowledge_cache_state");
      if (!kcs || kcs.status !== "attested") {
        writeLog("scope-before", "runtime", {
          sessionID: input.sessionID,
          callID: input.callID,
          agent,
          ...
          level: "ERROR",
          event: "TOOL-BEFORE",
          detail: "BLOCKED | UC7-001 | kcs=" + (kcs?.status || "unattested"),
        });
        throw new Error(
          "[FW-ENFORCE][UC7-001] Agent " +
            agent +
            " must call knowledge_cache_search()+knowledge_cache_attest() before modify tools. kcs=" +
            (kcs?.status || "unattested"),
        );
      }
    }
    ...
    const scopeResult = getEffectivePathScopePaths(
      input.tool,
      output.args || {},
    );
    ...
    DIAG(
      `CRASH ${err?.name || "Err"}: ${err?.message} | stack=${String(
        err?.stack || "",
      )
        .split("\n")
        .slice(0, 4)
