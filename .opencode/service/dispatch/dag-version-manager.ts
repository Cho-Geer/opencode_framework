...
import type { DAGTask, DAGMeta } from "../../lib/state-manager";
import { STATE_PATHS, getDateKey } from "../../lib/state-manager";
import { writeLog } from "../../lib/log-manager";
import { atomicWriteJson, atomicWriteText } from "../../lib/state-utils";
...
export class DAGVersionManager {
  ...
  async createVersionSnapshot(
    ...
    const pendingCount = tasksToKeep.filter(
      (t) => t.status === "pending",
    ).length;
    const recentCompleted = tasksToKeep.filter(
    ...
    dag.meta.archived_completed =
      (dag.meta.archived_completed || 0) + archivedCount;
  ...
  getVersionHistory(): Array<{
    ...
    try {
      ...
      const files = readdirSync(this.versionsDir)
        .filter(
          (f: string) => f.startsWith("Task.DAG.v") && f.endsWith(".json"),
        )
        .sort();
      ...
      for (const line of lines) {
        ...
        if (versionMatch) {
          ...
          currentEntry.summary +=
            (currentEntry.summary ? " " : "") + line.substring(2);
        }
      }
  ...
  /**
   * Write the DAG back to the hot file (atomic temp+rename).
   */
  private writeDAG(dag: DAGDocument): void {
    try {
      atomicWriteJson(this.hotFile, dag);
      writeLog("dag-version-manager", "runtime", {
        event: "DAG-WRITE-ATOMIC",
        detail: `hotFile=${this.hotFile}`,
      });
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private writeSnapshot(snapshotPath: string, dag: DAGDocument): void {
    ...
    try {
      atomicWriteJson(snapshotPath, dag);
      writeLog("dag-version-manager", "runtime", {
        event: "DAG-SNAPSHOT-ATOMIC",
        detail: `snapshotPath=${snapshotPath}`,
      });
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to write DAG snapshot ${snapshotPath}: ${message}`,
      );
    }
  }

  /**
   * Append a changelog entry to the markdown file (read-modify-write atomic).
   */
  private appendChangelog(version: string, summary: string): void {
    const dateKey = getDateKey();
    const entry = `\n## v${version} (${dateKey})\n- ${summary}\n`;

    // Read-modify-write atomic: read existing content, append in memory, then atomic write
    if (!existsSync(this.changelogFile)) {
      const header = `# Task.DAG Changelog\n\n> Auto-generated by DAGVersionManager. Do not edit manually.\n\n`;
      atomicWriteText(this.changelogFile, header + entry);
    } else {
      const content = readFileSync(this.changelogFile, "utf8");
      const headerEnd = content.indexOf("\n## ");
      if (headerEnd >= 0) {
        const newContent =
          content.slice(0, headerEnd) + entry + content.slice(headerEnd);
        atomicWriteText(this.changelogFile, newContent);
      } else {
        atomicWriteText(this.changelogFile, content + entry);
      }
    }
  }
