/**
 * backup.ts — Git-backed backup management with DB-only logging
 * =============================================================
 *
 * Extracted from backup-manager.ts during FileGuard Service migration (Phase 1a).
 *
...
export function restoreBackup(uuid: string): {
  ...
  // Update DB — mark original row as restored
  const db = getDb();
  const now = new Date().toISOString();
  db.run(
  ...
  // GAP-C-RESTORE-AUDIT (2026-06-27, @Super-Admin):
  // Use a unique UUID for the audit row to avoid UNIQUE constraint
  // on backup_log.uuid. The restore event is recorded as a new row
  // with a distinct uuid, linked via original_file_path.
  try {
    const auditUuid = uuid + "-restored-" + Date.now();
    db.run(
    ...
    // Non-fatal: audit row insertion failure does not block restore
    writeLog(SRC, "WARN", {
      event: "BACKUP-RESTORE-AUDIT-FAILED",
      detail: "uuid=" + uuid + " | " + (auditErr.message || "unknown"),
    });
  }
