...
#### 5.2.6 Admin Dashboard 集成测试 [v2.1.0]
...
| **DASH-002** | 验证预约趋势返回 `labels`/`bookingsData`/`revenueData` 数组且长度一致 | 集成测试 (Backend) | HTTP 200，`data.labels.length === data.bookingsData.length === data.revenueData.length` |
...
| **DASH-FE-002** | 验证 Chart.js 趋势图绑定 `labels`/`bookingsData`/`revenueData` 正确 | 组件测试 (Frontend) | Chart.js 实例接收正确数据集 |
...
describe('Admin Dashboard Stats (e2e)', () => {
  describe('GET /v1/admin/stats', () => {
    ...
    it('DASH-002: 预约趋势数组长度应一致', async () => {
      const response = await request(app.getHttpServer())
        .get('/v1/admin/stats/booking-trends?range=weekly&granularity=day')
        .set('Authorization', `Bearer ${adminToken}`)
        .expect(200);

      const { labels, bookingsData, revenueData } = response.body.data;
      expect(labels.length).toBeGreaterThan(0);
      expect(labels.length).toBe(bookingsData.length);
      expect(labels.length).toBe(revenueData.length);
    });
