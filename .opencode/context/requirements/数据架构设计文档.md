...
model TimeSlot {
  id               String   @id @default(uuid())
  slotTime         String   @unique // 时间段（格式："HH:MM"，24小时制，使用UTC存储避免时区混淆）
  durationMinutes  Int      @default(60) // 持续时间（分钟）
  capacity         Int      @default(1)  // 槽位容量（支持多人预约）
  currentSequence  Int      @default(0)  // 当前已分配的序号（原子化抢占，必须使用数据库原子操作递增）
  ...
  @@index([isActive])
  @@index([displayOrder])
  @@index([capacity, currentSequence], where: raw("\"isActive\" = true"))
}

// **关键实现约束**：`currentSequence` 字段的递增必须使用数据库的原子操作
// （如 PostgreSQL 的 `UPDATE ... SET currentSequence = currentSequence + 1 WHERE ... RETURNING currentSequence`），
...
| **唯一约束** | `@unique` | `phone String? @unique` | 应用层缓存唯一性检查 |
...
|---------|---------|---------|------|---------|
| `phone` | User | 唯一索引 | 手机号唯一性校验 | 应用层缓存检查 |
| `email` | User | 唯一索引 | 邮箱唯一性校验 | 应用层缓存检查 |
| `appointmentNumber` | Appointment | 唯一索引 | 预约编号唯一性 | 前缀索引优化查询 |
| `slotTime` | TimeSlot | 唯一索引 | 时间段唯一性 | 业务层缓存 |
...
- **更新记录**: 2026-04-14 - 基于MCP获取的最新技术栈信息更新；2026-04-21 - TASK-D3 PII 加密三字段模型
