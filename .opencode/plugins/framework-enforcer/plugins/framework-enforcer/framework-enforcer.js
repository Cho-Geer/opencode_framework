...
function findArmedSession() {
    ...
    const armedSession = sessions.find((s) => s.gate_status === "armed" && s.consumed_at === null);
    if (armedSession) {
...
function isStaleSession(session) {
    if (!session.confirmed_at || session.consumed_at)
        return false;
...
function autoDrainStaleSessions(paths) {
    try {
        ...
        if (!gate?.sessions)
            return 0;
        const staleEntries = Object.entries(gate.sessions).filter(([, s]) => isStaleSession(s));
        ...
        gate.active_sessions = (gate.active_sessions || []).filter((sid) => gate.sessions[sid] && gate.sessions[sid].gate_status === "armed");
...
function checkRuleRegistryIntegrity(paths) {
    try {
        const registry = readJsonFile(resolveStatePath(".opencode/state/rule_registry.json"));
        if (!registry)
...
async function toolExecuteBefore(input, output) {
    ...
    if (!integrityResult.valid) {
        if (mode === "strict" || mode === "locked") {
            throw new Error(`[FW-ENFORCE] Plugin integrity violation: ${integrityResult.detail}`);
        }
        logAuditEntry({
            event: "silent_audit",
    ...
    if (taskId) {
        ...
        else if (taskCheck.status !== "pending" &&
            taskCheck.status !== "in_progress") {
            violations.push(`[FW-ENFORCE] Task "${taskId}" has status "${taskCheck.status}" (expected pending/in_progress)`);
        }
    }
    ...
    if (isModifyingTool && !isBootstrapTool) {
        ...
        if (!armedCheck.found && mode !== "advisory") {
            violations.push(`[FW-ENFORCE] No armed compliance gate session found (mode: ${mode})`);
        }
    }
    // ---- Check write scope ----
    if ((tool === "write" || tool === "edit") && agent) {
        ...
        if (filePath && !isWriteAllowed(agent, filePath)) {
            if (mode === "strict" || mode === "locked") {
                violations.push(`[FW-ENFORCE] Agent "${agent}" write to "${filePath}" blocked by agent_write_scopes (mode: ${mode})`);
            }
        }
    }
            ...
            if (newContent) {
                try {
                    ...
                    if (newMode && newMode !== mode) {
                        violations.push(`[FW-ENFORCE] Blocked attempt to change enforcement_mode from "${mode}" to "${newMode}" in ${mode} mode`);
                    }
                }
                catch {
    ...
    if (tool === "bash") {
        ...
        if (/rm\s+.*\.opencode|mv\s+.*\.opencode|chmod\s+.*777|>\s*\.opencode|sudo\s+rm/i.test(cmd)) {
            if (mode !== "advisory") {
                violations.push(`[FW-ENFORCE] Dangerous bash command blocked: ${cmd.slice(0, 80)}`);
            }
            else {
                logAuditEntry({
                    timestamp: new Date().toISOString(),
    ...
    if (!registryCheck.valid) {
        ...
        if (mode !== "advisory") {
            violations.push(`[FW-ENFORCE] Rule registry has ${registryCheck.mismatches.length} HIGH mismatches`);
        }
    }
    ...
    if (!machineCheck.clean && mode !== "advisory") {
        violations.push(`[FW-ENFORCE] Machine.json dirty: ${machineCheck.dirty.join(", ")}`);
    }
        ...
        if (gate?.sessions) {
            ...
            for (const stale of staleSessions) {
                writeAuditLogEntry({
                    ...
                    hours_stale: ((Date.now() - new Date(stale.confirmed_at).getTime()) /
                        3600000).toFixed(1),
                });
            }
        }
...
async function fileEdited(input, _output) {
    ...
    if (!isCriticalFrameworkFile(filePath))
        return;
    if (mode === "locked") {
        throw new Error(`[FW-ENFORCE][LOCKED] Tamper blocked: critical framework file "${filePath}" edited by "${agent}". Auto-restore attempted.`);
    }
    if (mode === "strict") {
        logAuditEntry({
            timestamp: new Date().toISOString(),
...
async function permissionAsked(input, _output) {
    ...
    for (const ep of escalationPatterns) {
        if (ep.pattern.test(tool)) {
            ...
            logAuditEntry({
                event: "silent_audit",
                detail: mode === "strict" || mode === "locked"
...
async function tuiCommandExecute(input, _output) {
    ...
    for (const dc of dangerousCommands) {
        if (lower.startsWith(dc)) {
            ...
            if (mode === "strict" || mode === "locked") {
                blocked = true;
                throw new Error(`[FW-ENFORCE] Dangerous TUI command blocked: ${input.command}`);
            }
