...
describe("state-manager", () => {
  ...
  describe("buildArchiveRef()", () => {
    it("builds correct reference string", () => {
      expect(buildArchiveRef("2026-06-03", 5)).toBe(
        "gate-state.history/2026-06-03.jsonl#5",
      );
    });

    it("handles index 0", () => {
      expect(buildArchiveRef("2026-01-01", 0)).toBe(
        "gate-state.history/2026-01-01.jsonl#0",
      );
    });
  });
  ...
  describe("parseArchiveRef()", () => {
    ...
    it("round-trips with buildArchiveRef", () => {
      ...
      expect(parseArchiveRef(ref)).toEqual({
        filename: "2026-12-25.jsonl",
        lineIndex: 42,
      });
    });
  });
