...
import { writeFileSync, readFileSync, existsSync, rmSync, mkdtempSync, mkdirSync, readdirSync } from 'node:fs';
...
describe('state-compactor', () => {
  ...
  describe('onGateComplete()', () => {
    ...
    it('creates index entry', async () => {
      ...
      expect(idx.sessions['cg_ses_1111111111111'].archive_ref).toMatch(/gate-state\.history\/.*\.jsonl#\d+/);
    });
  });
  ...
  describe('drainStaleSessions()', () => {
    beforeEach(() => {
      createHot(tmpDir, {
        ...
        meta: { total_sessions: 2, active_count: 2, recent_count: 0, last_compacted: new Date().toISOString() },
      });
    });
    ...
    it('drains stale sessions', async () => {
      ...
      const hot = JSON.parse(readFileSync(join(tmpDir, 'gate-state.json'), 'utf8'));
      // active_sessions should be empty since the session was not "active" status
      expect(Object.keys(hot.active_sessions).length).toBe(0);
    });
  ...
  describe('history JSONL writing', () => {
    it('appends to existing JSONL file', async () => {
      ...
      const s2 = { ...testSession, session_id: 'cg_ses_2222222222222', task_description: 'Second task' };
      createHot(tmpDir, {
        active_sessions: { ['cg_ses_2222222222222']: s2 },
        meta: { total_sessions: 2, active_count: 1, recent_count: 1, last_compacted: new Date().toISOString() },
      });
      writeFileSync(join(tmpDir, 'gate-state.index.json'), JSON.stringify({
        formatVersion: '3.0',
        sessions: { 'cg_ses_1111111111111': { session_id: 'cg_ses_1111111111111', archive_ref: 'gate-state.history/' + new Date().toISOString().split('T')[0] + '.jsonl#0' } },
      }));
