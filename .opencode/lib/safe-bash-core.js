...
export const ALLOWED_SCRIPT_PATHS = ['__tests__', '.opencode_backups', '.task_temp'];
 ...
 */
export function matchGlob(command, pattern) {
    const regex = pattern
        .replace(/\*/g, '.*')
        .replace(/\?/g, '.')
        .replace(/\[/g, '\\[')
 ...
 */
export function getAllowlist(agent) {
 ...
 */
export function _scriptContainsFileWrite(scriptPath) {
    let content;
    try {
 ...
 */
export function _isScriptInAllowedPath(scriptPath) {
    const normalized = path.resolve(scriptPath).replace(/\\/g, '/');
    return ALLOWED_SCRIPT_PATHS.some((allowed) => normalized.includes(`/${allowed}/`) ||
 ...
 */
export function safeBashTool(options) {
    const { command, timeout = 300000, dryRun = false, agent = process.env.FRAMEWORK_AGENT || 'unknown', } = options;
