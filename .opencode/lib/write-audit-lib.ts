...
// readJsonFile inlined to avoid OpenCode plugin runtime import issue
...
export function executeWriteAuditCheck(
  ...
  try { machine = JSON.parse(fs.readFileSync(machinePath, "utf8")); } catch { return; }
  if (!machine) return;
