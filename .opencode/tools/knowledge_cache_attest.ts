...
export default tool({
  description:
    "Agent-submitted read evidence attestation. Verifies that the agent actually read the declared cache files (via read tool + read_audit.jsonl cross-check) and formally attests the domain's knowledge readiness. REQUIRED before writing source files in strict/locked mode (UC7-001 Phase 0).",
  args: {
    domain: tool.schema
      .string()
    ...
    return withInterruptGuard("knowledge_cache_attest", async function () {
      ...
      try {
        writeOk = atomicWriteSubState("knowledge_cache_state", function (state: any) {
          ...
          var ok = writeCacheAttestation(state.session_access, agentRef, taskId, domain, attestation);
          if (!ok) {
      ...
      writeLog(SRC, "INFO", {
        ...
        detail: `taskId=${taskId} domainId=${domain} files=${filesRead.length}`,
      });
