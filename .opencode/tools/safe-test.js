...
exports.validateTestReport = validateTestReport;
 ...
 */
function _readReport(taskId) {
    const reportPath = path.join(process.cwd(), ".task_temp", taskId, REPORT_FILENAME);
    const violations = [];
 ...
 */
function _validateCoverage(report) {
    const violations = [];
 ...
 */
function validateTestReport(taskId, phase) {
    const violations = [];
    ...
    if (rep.execution_evidence === undefined ||
