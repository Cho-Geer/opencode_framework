...
- **Step 8**: `codegraph-enforce.ts` plugin 实现硬约束——拦截 safe_edit/safe_delete/safe_restore/safe_shell 4 个工具，要求先执行 `codegraph_impact` 分析。`INTERCEPTED_TOOLS` Set + session 级状态追踪 + `extractFilePath()` 按工具类型提取目标路径。Plugin 注册到 `opencode.json` 的 `plugin` 数组
