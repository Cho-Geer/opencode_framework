...
#### HANDOVER.md required section: Findings

Your HANDOVER.md MUST include a `## Findings` section with a table of issues
discovered during task execution. This table populates the `findings_reported`
parameter of `compliance_gate_approve_deliverables`.
...
| -------- | ------------- | ---------------------------- |
| ⚠️       | category_name | Brief description of finding |
| ❌       | category_name | Brief description of finding |
```

- **`findings_reported`**: A pipe-separated (`|`) string of severity values
  found in your Findings table. Example: `"missing_state|over_blocking"`.
- Only enforced when both `findings_reported` is provided AND the `## Findings`
...
- Declare each concrete output you will produce as a JSON array:
  `[{"name":"HANDOVER.md","description":"Handover summary","artifact_path":".task_temp/{taskId}/HANDOVER.md","required":true},{"name":"TASK_LOG.md","description":"Working memory log","artifact_path":".task_temp/{taskId}/TASK_LOG.md","required":true}]`
- **Exempt agents** (@Orchestrator, @Super-Admin): may omit `declared_deliverables`.

> **Optimization**: If you call `compliance_gate_check(task_description, task_id, plan_summary=...)` with a `plan_summary` (min 10 chars), the gate is created AND armed in a single call — you do NOT need a separate `compliance_gate_confirm` call. This combined flow is the recommended approach for all agents.

### Step 2c: Gate Recovery (self-heal)

If `compliance_gate_submit_deliverables` returns `recoverable` status (artifacts written out of order), call:
`compliance_gate_retry_confirm(session_id, plan_summary, task_id)` to re-arm the session, then re-submit deliverables.

### Step 2d: Knowledge-Curator Combined Gate Flow

@Knowledge-Curator uses a specialized combined gate flow (see `dispatch-subagent.ts` L712–736). When dispatched by @Orchestrator or @Super-Admin for knowledge acquisition tasks:

1. `compliance_gate_check` with `plan_summary` auto-arms the gate
2. KC fetches and caches documentation
3. KC calls `compliance_gate_submit_deliverables(session_id, evidence)` to transition to `delivered`
4. The dispatching agent (Orchestrator/Super-Admin) calls `compliance_gate_approve_deliverables(session_id, "approve", note, execution_summary, handover_sha256)` to close
...
**4a.** Write **HANDOVER.md** and **TASK_LOG.md** (both mandatory for all agents), plus any other declared deliverables.
...
- **Non-exempt agents**: Your session ends after `submit_deliverables`. The Orchestrator will review your deliverables and call `compliance_gate_approve_deliverables(session_id, "approve"|"reject", note, execution_summary, handover_sha256)` to close the gate on your behalf.
  - `handover_sha256` is **REQUIRED**: SHA-256 hash of HANDOVER.md content. Compute via: `sha256sum .task_temp/{taskId}/HANDOVER.md`
  - The server verifies this hash against the actual file — proving the approver actually READ the deliverables.
  - If the hash is missing or wrong, the approve call will be REJECTED.

### Session Management: Drain Stale Gates

Orchestrator or Super-Admin may call `compliance_gate_drain_stale` to clean up expired sessions:

- `compliance_gate_drain_stale()` — default thresholds: armed > 24h, checked > 48h
- Optional: `compliance_gate_drain_stale(threshold_hours_armed, threshold_hours_checked)` for custom thresholds
- Drained sessions are moved to `gate-state.json.drained_sessions` preserving audit trail
