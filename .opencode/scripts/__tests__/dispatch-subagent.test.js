...
// --- Helper: run dispatch-subagent.js in child with --task-id flag ---
function runAndCaptureEnv(agentType, taskDesc, taskId) {
  // Pattern: node dispatch-subagent.js <agent_type> "<task_description>" [--task-id <id>]
  var argvItems = [JSON.stringify(agentType), JSON.stringify(taskDesc)];
  if (taskId !== null && taskId !== undefined) {
    argvItems.push('"--task-id"', JSON.stringify(taskId));
  }
