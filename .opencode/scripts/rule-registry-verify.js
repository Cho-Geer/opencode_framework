...
const PRIMARY_REGISTRY_PATH = path.join(PROJECT_ROOT, '.opencode', 'state', 'rule_registry.json');
// Fallback: some projects have it directly under .opencode/
const FALLBACK_REGISTRY_PATH = path.join(PROJECT_ROOT, '.opencode', 'rule_registry.json');
...
function main() {
  ...
  if (!fs.existsSync(REGISTRY_PATH)) {
    console.log(JSON.stringify({
      total: 0, valid: 0, violations: [{ severity: 'HIGH', issue: 'registry_missing', detail: 'rule_registry.json not found' }],
      summary: 'rule_registry.json not found'
    }, null, 2));
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(JSON.stringify({
      total: 0, valid: 0, violations: [{ severity: 'HIGH', issue: 'registry_invalid_json', detail: e.message }],
      summary: 'rule_registry.json is not valid JSON'
    }, null, 2));
    process.exit(1);
  }
  ...
  for (const key of entryKeys) {
    ...
    if (!entry || typeof entry !== 'object') {
      violations.push({ severity: 'HIGH', key, file: key, issue: 'invalid_entry', detail: 'Entry is not a valid object' });
      continue;
    }
    ...
    const missingRec = recommendedFields.filter(f => !entry[f] && !entry.digest_history);
    ...
    if (!fs.existsSync(filepath)) {
      violations.push({
        severity: 'HIGH', key, file: key, filepath: entry.path || key,
        issue: 'file_missing',
    ...
    if (actualHash !== storedHash) {
      ...
      if (currentSemver && storedSemver && currentSemver !== storedSemver) {
        ...
        violations.push({
          severity: 'WARNING', key, file: key, filepath: entry.path || key,
      ...
      if (shouldRepair) {
        const newSemver = currentSemver && storedSemver && currentSemver !== storedSemver
