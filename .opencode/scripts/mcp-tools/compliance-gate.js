...
const MCP_INVENTORY = process.env.MCP_INVENTORY_PATH ||
  require('path').join(OPENCODE_ROOT, '.opencode', 'rules', 'rule_detail', 'mcp-tool-inventory.md');

const COMMON_RULES = process.env.COMMON_RULES_PATH ||
  require('path').join(OPENCODE_ROOT, '.opencode', 'rules', 'common-project.md');

const SKILL_FILE = process.env.SKILL_FILE_PATH ||
  require('path').join(OPENCODE_ROOT, '.opencode', 'skills', 'execution-preflight-check', 'SKILL.md');
...
function getFreshStore() {
  return { formatVersion: '2.0', sessions: {} };
}
...
function runGateConfirm(sessionId, planSummary) {
  ...
  if (!session) {
    return { status: 'rejected', reason: `session not found: ${sessionId || '(missing)'}. Must call compliance_gate_check first.` };
  }
...
function runGateComplete(sessionId, executionSummary) {
  ...
  const session = sessionId ? store.sessions[sessionId] : null;
  if (!session) {
    return { status: 'rejected', reason: `session not found: ${sessionId || '(missing)'}. Must call compliance_gate_check and compliance_gate_confirm first.` };
  }
  if (session.gate_status !== 'armed') {
    return { status: 'rejected', reason: `session ${sessionId} is not armed (status: ${session.gate_status}). Must call compliance_gate_confirm first.` };
  }
  ...
  tools: [
    {
      ...
      description: 'MANDATORY runtime compliance gate. Must be called BEFORE any task execution. Verifies: (1) execution-preflight-check skill exists, (2) rule documents are present. Returns passed=true and session_id only when all checks clear. NOTE: Gate status is informational only - check does not require pre-armed gate, making it safe for concurrent sessions.',
      inputSchema: {
        type: 'object',
        properties: {
          task_description: {
    ...
    },
    {
      ...
      description: 'Mark the compliance gate as "armed" after the user has reviewed and confirmed the task plan. This must be called AFTER compliance_gate_check passes and AFTER the user explicitly confirms the plan. Provide the session_id returned by compliance_gate_check.',
      inputSchema: {
        type: 'object',
        properties: {
          session_id: {
            type: 'string',
            description: 'Session ID returned by compliance_gate_check (e.g. cg_ses_1777110280157)'
          },
          plan_summary: {
            type: 'string',
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === 'compliance_gate_complete') {
    if (!args?.session_id || !args?.execution_summary) {
      throw new Error('Missing required parameters: session_id and execution_summary');
    }
