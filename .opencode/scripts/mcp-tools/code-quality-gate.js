...
function checkScope(changedFile, agentType, machine) {
  ...
  if (!scopes || !scopes[agentType]) {
    return { status: 'error', violation: true, message: `No write scope defined for ${agentType}` };
  }
  const scope = scopes[agentType];
  // Normalize path (strip leading slashes, handle absolute paths)
  const projectRoot = getProjectRoot();
  let normFile = changedFile;
  if (normFile.startsWith(projectRoot + '/') || normFile.startsWith(projectRoot)) {
  ...
  for (const deny of scope.denied || []) {
    if (matchGlob(normFile, deny)) {
      ...
      return { status: 'fail', violation: true, severity: 'BLOCKER',
...
function checkFormat(changedFile, projectRoot, autoFix) {
  ...
  try {
    execSync(`npx prettier --check "${filePath}"`, { cwd: projectRoot, encoding: 'utf8', timeout: 5000, stdio: ['pipe', 'pipe', 'pipe'] });
    return { status: 'pass', formatted: true };
  } catch {
    if (autoFix) {
      try {
        execSync(`npx prettier --write "${filePath}"`, { cwd: projectRoot, encoding: 'utf8', timeout: 5000, stdio: ['pipe', 'pipe', 'pipe'] });
        return { status: 'pass', formatted: true, auto_fixed: true };
      } catch {
        return { status: 'fail', formatted: false, message: 'Prettier check failed and auto-fix failed' };
      }
    }
...
function checkDeps(changedFile, projectRoot) {
  ...
  try {
    const result = execSync(`npx depcruise --include-only "^${filePath}" --output-type json "${projectRoot}"`, {
      cwd: projectRoot, encoding: 'utf8', timeout: 10000, stdio: ['pipe', 'pipe', 'pipe']
    });
    const data = JSON.parse(result);
    if (data.summary?.violations?.length > 0) {
      return { status: 'fail', violations: data.summary.violations, count: data.summary.violations.length };
    }
    ...
    if (e.message && e.message.includes('Cannot find module')) {
      return { status: 'skip', message: 'dependency-cruiser not installed. Run: npm install --save-dev dependency-cruiser' };
    }
    ...
    try {
      ...
      const violations = results
        ...
        .flatMap(f => f.messages.map(m => ({
          file: f.filePath, line: m.line, rule: m.ruleId, message: m.message, severity: m.severity
        })));
      ...
      return { status: 'error', message: e.message?.substring(0, 200) || 'ESLint error' };
    }
  }
}
...
function checkTDDOrder(machine, changedFile) {
  ...
  const isTestFile = fileName.includes('.spec.') || fileName.includes('.test.') || changedFile.includes('/test/');
  const isImplFile = /\.(ts|js)$/.test(changedFile) && !isTestFile && !changedFile.includes('.config.');
  const isConfigOrDoc = /\.(json|yaml|yml|md)$/.test(changedFile);

  if (!machine.tdd_enforcement_state) {
    machine.tdd_enforcement_state = {
      enabled: true,
      current_session: { test_written: false, impl_files_attempted: [], blocked_attempts: [], test_files_written: [] },
      violations: [], history: []
    };
  }
  ...
  if (isTestFile || isImplFile) {
    if (!state.current_session.initialized) {
      state.current_session = { test_written: false, impl_files_attempted: [], blocked_attempts: [], test_files_written: [], initialized: true };
    }
  }
  ...
  if (isTestFile) {
    ...
    state.current_session.test_files_written.push(changedFile);
    return { status: 'pass', message: `Test file recorded: ${fileName}` };
  }

  if (isImplFile) {
    if (!state.current_session.test_written) {
      state.current_session.impl_files_attempted.push(changedFile);
      state.current_session.blocked_attempts.push({ file: changedFile, timestamp: new Date().toISOString() });
...
function checkTsc(changedFile, projectRoot) {
  ...
  try {
    ...
    return { status: 'fail', errors: 1, time_ms: elapsed, message: e.stderr?.substring(0, 300) || e.stdout?.substring(0, 300) || 'TypeScript error' };
  }
}
...
function updateStates(machine, results, agentType, taskId, file) {
  ...
  const session = machine.write_audit_state.current_session || {
    agent: agentType, task_id: taskId || 'unknown', files_written: [],
    checks_run: 0, checks_passed: 0, checks_failed: 0, violations_found: 0, violations_resolved: 0, scope_violations_attempted: 0
  };
  ...
  if (results.checks.deps) {
    ...
    if (results.checks.deps.status === 'fail') {
      machine.dependency_state.violations.push(...(results.checks.deps.violations || []));
...
function runWriteCheck(params) {
  ...
  if (!skip.has('format')) {
    ...
    if (r.auto_fixed) results.fixes_applied.push({ check: 'format', action: 'prettier --write' });
    if (r.status === 'fail') {
      results.overall = 'fail';
  ...
  if (!skip.has('eslint')) {
    ...
    if (r.tier1_mock_count > 0) {
      results.overall = 'fail';
      results.violations.push({ check: 'eslint', severity: 'BLOCKER', message: `CAT1.1: ${r.tier1_mock_count} Tier1 service mock(s) detected` });
    } else if (r.status === 'fail') {
      results.violations.push({ check: 'eslint', severity: 'ERROR', message: `${r.violations?.length || 0} ESLint violations` });
    }
  }
  ...
  if (!skip.has('tsc')) {
    ...
    if (r.status === 'fail') {
      results.overall = 'fail';
      results.violations.push({ check: 'tsc', severity: 'BLOCKER', message: r.message });
    }
  }
...
function runFullScan() {
  ...
  for (const cwd of [beCwd, feCwd]) {
    try {
      execSync('npx tsc --noEmit --pretty false', { cwd, encoding: 'utf8', timeout: 60000, stdio: ['pipe', 'pipe', 'pipe'] });
    } catch (e) {
      tscErrors++;
      results.overall = 'fail';
      results.violations.push({ check: 'tsc_full', severity: 'BLOCKER', message: `TypeScript errors in ${path.basename(cwd)}` });
    }
  }

  // Check #2: dependency-cruiser full scan
  try {
    execSync('npx depcruise --config .dependency-cruiser.js --output-type json .', {
    ...
    try {
      ...
      if (depsViolations > 0) {
        ...
        machine.dependency_state.status = 'dirty';
        results.overall = 'fail';
        results.violations.push({ check: 'dep_full', severity: 'ERROR', message: `${depsViolations} dependency violations` });
      }
    } catch {}
  }
  ...
  try {
    ...
    machine.format_state.status = 'dirty';
    results.overall = 'fail';
    results.violations.push({ check: 'format_full', severity: 'ERROR', message: 'Some files are not formatted' });
  }
  ...
  tools: [
    {
      ...
      description: 'Write-Time audit on a single changed file (Layer A). Checks: Agent Scope, Prettier, dependency-cruiser, ESLint mock-audit, tsc incremental.',
      inputSchema: {
        type: 'object',
        required: ['changed_file', 'agent_type'],
        properties: {
          changed_file: { type: 'string', description: 'Path of the changed file (absolute or project-relative)' },
          agent_type: { type: 'string', enum: ['@Coder-BE', '@Coder-FE', '@Architect', '@Orchestrator', '@Guardian', '@Meta-Planner'], description: 'Agent identity for write scope enforcement' },
          skip_checks: { type: 'array', items: { type: 'string', enum: ['scope', 'format', 'deps', 'eslint', 'tsc', 'tdd'] }, description: 'Optional checks to skip' },
          auto_fix: { type: 'boolean', description: 'Auto-fix formatting via prettier --write (default: true)' },
          task_id: { type: 'string', description: 'Current task ID for audit trail' }
        }
      }
    },
    {
      name: 'run_full_scan',
      description: 'Full project scan (Layer B). Runs: tsc full, depcruise full, prettier full. Updates machine.json states.',
      inputSchema: {
        type: 'object',
        properties: {}
      }
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === 'run_write_check') {
    if (!args?.changed_file || !args?.agent_type) {
      throw new Error('Missing required parameters: changed_file and agent_type');
    }
