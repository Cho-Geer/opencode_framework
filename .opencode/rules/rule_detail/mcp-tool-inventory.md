...
**制定时间**: 2026-04-10  
**最后更新**: 2026-06-21  
**版本**: v3.0.0（KC-04：修正术语——区分MCP工具与OpenCode自定义工具；新增0.1节术语说明和1.13节自定义工具清单）

---

## 术语说明

OpenCode 项目中有**三类工具**，调用方式不同：

| 类别                                   | 定义                                                          | 存放位置                       | 工具命名                                           | 示例                                                                                      |
| -------------------------------------- | ------------------------------------------------------------- | ------------------------------ | -------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **OpenCode内置工具**                   | OpenCode 框架自身提供的工具                                   | 框架内置                       | 直接使用工具名                                     | `read`, `write`, `edit`, `bash`, `grep`, `glob`, `question`, `todowrite`, `skill`, `task` |
| **MCP工具（MCP Servers）**             | 通过 Model Context Protocol 连接的外部服务器提供的工具        | `opencode.json` → `mcp` 段声明 | `<server-name>_<tool-name>`                        | `compliance-gate_compliance_gate_check`, `context7_query-docs`, `github_search_code`      |
| **OpenCode自定义工具（Custom Tools）** | 项目通过 `tool()` from `@opencode-ai/plugin` 定义的自定义函数 | `.opencode/tools/*.ts`         | 文件名（单导出）或 `filename_exportname`（多导出） | `config_read_attest`, `knowledge_cache_search`, `safe_edit`, `dispatch_subagent`          |

> **重要区分**：MCP工具 ≠ 自定义工具。MCP工具在 `opencode.json.mcp` 中声明服务器，命名遵循 `<server>_<tool>` 格式。自定义工具在 `.opencode/tools/` 目录中定义，使用 `tool()` API，命名等于文件名。本文档第1节专门列出MCP工具，第1.13节列出OpenCode自定义工具。

---

## 一、当前项目可用MCP工具清单（opencode.json.mcp 注册的MCP服务器）
...
### 1.10 Keystone Validate CLI工具（非MCP）
...
> **注**：`keystone_validate` 存放于 `.opencode/scripts/mcp-tools/` 目录，但**未在 `opencode.json.mcp` 中注册为MCP服务器**，因此不是MCP工具。它通过 `safe_shell` 以CLI方式调用执行。
...
### 1.12 Code Quality Check MCP工具（v4.0.0 — replaces deprecated code-quality-gate）

| 工具名称                                                   | 功能描述                                                                                                                                | 适用场景                                               |
| ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| `code_quality_check.run_depcruise_check({ changed_file })` | **单文件依赖边界检查** — 对单个修改文件运行 dependency-cruiser 架构边界验证                                                             | @Coder-BE/@Coder-FE 每次关键 Write/Edit 后调用         |
| `code_quality_check.run_tsc_check({ changed_file })`       | **单文件类型检查** — tsc --noEmit 增量类型检查                                                                                          | @Coder-BE/@Coder-FE 每次 Write/Edit 后调用             |
| `code_quality_check.run_full_scan()`                       | **全量扫描** — 执行 tsc全量类型检查 + depcruise全量依赖扫描 + prettier全量格式检查。用于`compliance_gate_complete`内部和pre-commit hook | `compliance_gate_complete`内部调用；Guardian审查前调用 |

> **Note**: Auto-formatting (Prettier) is now handled by the `format-after` plugin, which runs automatically on every write. No MCP call needed for format checks.

**调用方式**:

```
# Layer A — Write-Time（每次修改后立即执行）
code_quality_check.run_tsc_check({ changed_file: "src/modules/xxx/xxx.service.ts" })
code_quality_check.run_depcruise_check({ changed_file: "src/modules/xxx/xxx.service.ts" })

# Layer B — Full Scan（compliance_gate_complete/pre-commit）
code_quality_check.run_full_scan()
...
### 1.13 OpenCode自定义工具清单（.opencode/tools/ 目录下的 Custom Tools）

以下工具是通过 `tool()` from `@opencode-ai/plugin` 在 `.opencode/tools/*.ts` 中定义的**OpenCode自定义工具**，不是MCP工具。工具命名等于文件名。

| 工具名称                 | 功能描述                                                                                                        | 适用场景                              |
| ------------------------ | --------------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| `config_read_attest`     | 验证Agent已读取3个强制配置文件（agent config, opencode.json, project.config.json），写入config_read_state子状态 | Agent在P0 Step 0e调用，解锁写权限     |
| `dispatch_subagent`      | 生成包装后的合规Prompt，用于派遣子Agent                                                                         | @Orchestrator/@Super-Admin派遣子Agent |
| `knowledge_cache_attest` | Agent提交的读取证据证明，自声明缓存充足性（M9）。验证Agent实际读取了缓存文件（read + read_audit交叉检查）       | strict/locked模式下写源文件前强制调用 |
| `knowledge_cache_search` | 自动化本地知识缓存搜索。读取docs/official_docs/index.json，按domain/tags匹配条目                                | UC7KS Pipeline Step 0b                |
| `knowledge_gap_report`   | 分析所有语义领域的知识缓存覆盖率                                                                                | 缓存健康检查                          |
| `module_scope_declare`   | 声明当前任务的目标模块作用域，映射模块→知识领域→缓存路径→Context7库                                             | 任务开始时调用（Step 0a）             |
| `resolve_domain_id`      | 解析当前session的dispatch分配的知识领域ID                                                                       | Agent自检domain_id                    |
| `safe_edit`              | 安全原子文件编辑（TOCTOU保护、备份、回滚）                                                                      | 所有文件修改                          |
| `safe_shell`             | 白名单Shell命令执行                                                                                             | 所有shell操作                         |
| `safe_delete`            | 安全文件删除（TOCTOU保护、备份、回滚）                                                                          | 文件删除                              |
| `safe_mkdir`             | 安全目录创建                                                                                                    | 目录创建                              |
| `safe_restore`           | 从备份恢复文件                                                                                                  | 回滚文件修改                          |
| `safe_diff`              | 生成文件差异对比                                                                                                | 变更审查                              |
| `safe_test`              | 安全测试执行                                                                                                    | 测试运行                              |

> **注意**：以上是OpenCode自定义工具（Custom Tools），与MCP工具（如 `compliance-gate_*`、`code-quality-check_*`）不同。自定义工具在 `.opencode/tools/` 中定义，命名等于文件名；MCP工具通过 `opencode.json.mcp` 中声明的MCP服务器提供，命名遵循 `<server-name>_<tool-name>` 格式。
...
| **Write-Time Audit**    | **code-quality-check** (P0强制)          | eslint-audit           |
...
| **合规门禁（后）**             | `compliance_gate_complete` + `code_quality_check.run_full_scan()`                                                                            | 阻塞，未完成不得标记任务结束。读取 machine.json 全部 8 维状态，任一 dirty → failed                                                                           |
| **Write-Time Audit（P0强制）** | Auto-format via `format-after` plugin; `code_quality_check.run_tsc_check()` + `code_quality_check.run_depcruise_check()` for type/dep checks | **P0阻塞不可跳过**。每次 Write/Edit 后必须执行。5项检查：scope/format/deps/eslint/tsc（底层由 `code-quality-lib.js` 执行）。违规当场阻断。跳过 → CAT5.1 违规 |
| **测试编写/修改后**            | `eslint_audit.run_audit({ changed_file })`                                                                                                   | 建议非阻塞。提前发现 TIER1 mock 违规                                                                                                                         |
| **Guardian审查前**             | `code_quality_check.run_full_scan()`                                                                                                         | 阻塞。验证 write_audit_log 完整性                                                                                                                            |
