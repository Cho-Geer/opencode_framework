...
- **合规门禁升级**：仅高风险修改、正式交付、跨 Agent 协作或需可追溯审计的任务默认升级到 `compliance_gate_check(task_description)` → 展示计划待用户确认 → `compliance_gate_confirm(plan_summary)`。普通小任务沿用 `preflight-lite` 轻路径，不要求先武装 gate。
