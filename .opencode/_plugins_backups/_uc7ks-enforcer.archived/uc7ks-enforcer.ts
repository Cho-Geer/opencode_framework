...
async function toolExecuteBefore(
  ...
  /**
   * UC7KS-HARDEN-AGENT-001: Dispatch-Bypass Detection (mirrors framework-enforcer.ts)
   *
   * OpenCode plugins cannot natively identify agent/agent_type at runtime.
   * If FRAMEWORK_DISPATCH_CONTEXT is "orchestrated" but FRAMEWORK_AGENT is empty,
   * dispatch-subagent.ts was bypassed — block external queries in strict/locked mode.
   *
   * @since 2026-06-06
   */
  const dispatchContext = process.env.FRAMEWORK_DISPATCH_CONTEXT || "";
  if (dispatchContext === "orchestrated" && !agent) {
    const bypassMsg =
      `[UC7KS-Enforcer][AGENT-001] Dispatch bypass detected: sub-agent without ` +
      `FRAMEWORK_AGENT attempting external query "${tool}". Blocked.`;
    console.error(bypassMsg);

    if (mode !== "advisory") {
      throw new Error(buildUC7KSError(
        "(unknown)", tool, mode,
        isLocalCacheAvailable(),
        "AGENT-001: Dispatch bypass detected — FRAMEWORK_AGENT not set. " +
        "All external queries blocked for unidentified agents in " +
        mode.toUpperCase() + " mode.",
      ));
    }
    // advisory: log warning, allow execution
    console.warn(
      `[UC7KS-Enforcer][AGENT-001][ADVISORY] Dispatch bypass with external ` +
      `query "${tool}" — allowing in advisory mode but audit trail recorded.`
    );
  }
  ...
  //   strict:   Block if cache_sufficiency.status !== "insufficient"
  //   locked:   Block if cache_sufficiency.status !== "insufficient"
  if (agentHasReadCache && cacheAvailable && mode !== "advisory") {
    try {
      const machinePath2 = path.resolve(resolveProjectRoot(), ".opencode", "state", "machine.json");
      if (fs.existsSync(machinePath2)) {
        const machine = JSON.parse(fs.readFileSync(machinePath2, "utf8"));
        const sessionAccess2 = machine?.knowledge_cache_state?.session_access;
        const agentAccess2 = sessionAccess2?.[agent];
        const sufficiencyStatus = agentAccess2?.cache_sufficiency?.status;

        // MOD-7 FIX (FW-REPAIR-UC7KS-REMEDIATION): Block only when agent has NOT declared
        // cache sufficiency at all (i.e. "undeclared" or undefined/null).
        // "sufficient" → allowed (agent found what it needed in cache)
        // "insufficient" → allowed (agent declared what's missing per Step 0c)
        // "undeclared" / undefined / null → blocked (agent hasn't done Step 0c)
        if (sufficiencyStatus !== "sufficient" && sufficiencyStatus !== "insufficient") {
          const errorMsg = buildUC7KSError(
            agent, tool, mode, true,
            `UC7-001b VIOLATION: Cache sufficiency not declared. Agent must search local cache (Step 0b) and declare sufficiency status (Step 0c) before external queries. Current status: ${sufficiencyStatus || "undeclared"}. See subagent-preamble.md §Step 0c.`,
          );
          console.error(
            `[UC7KS-Enforcer][UC7-001b][${mode.toUpperCase()}] Agent "${agent}" ` +
            `completed cache read but has not declared cache sufficiency. ` +
            `Status: ${sufficiencyStatus || "undeclared"}. Require: declare "sufficient" or "insufficient" per Step 0c.`
          );
          throw new Error(errorMsg);
        }
      }
    } catch (e: any) {
      if (e.message?.includes("UC7-001b")) throw e; // Re-throw UC7-001b errors
      // Non-fatal: if machine.json is unreadable, fall through
    }
  }
