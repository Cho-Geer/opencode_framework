...
**状态**: ✅ 已完成 — 全部 14 项修复(M1-M14)已实施并通过集成验证 (2026-06-21)
...
以下 M1–M14 修复项已全部实施，对应代码已提交至仓库。本验证方案中的所有用例已参照实际代码行为更新，无需逐项人工执行验证（各修复项的实现本身即为验证）。

### P0 — Write-Block 绕过级（全部完成）

|    #    | 状态 | 实施内容                                                                                                           |
| :-----: | :--: | ------------------------------------------------------------------------------------------------------------------ |
| **M1**  |  ✅  | `Super-Admin.md` mcp_tools 已添加 `knowledge_cache_attest`                                                         |
| **M2**  |  ✅  | `subagent-preamble.md` Step 0b+ 已升级为强制 `knowledge_cache_attest` 步骤                                         |
| **M3**  |  ✅  | `checkUC7KSWrite()` Path C 已改为遍历 session 所有 domain，每个 domain 的 attestation.status === "attested" 才放行 |
| **M11** |  ✅  | 文件级 domain 检查已实现：根据 filePath 匹配 save_path 找到目标 domain，未 attest → BLOCK                          |

### P1 — 域一致性 + 数据完整性（全部完成）

|    #    | 状态 | 实施内容                                                                                                                          |
| :-----: | :--: | --------------------------------------------------------------------------------------------------------------------------------- |
| **M4**  |  ✅  | `module_scope_declare` 调用时同步更新 DB session_map.domain_id（早期已完成）                                                      |
| **M5**  |  ✅  | Path B Fix v3：仅当已有 >=1 attested domain 时才容忍域不匹配                                                                      |
| **M6**  |  ✅  | `subagent-preamble.md` Step 0a `resolve_domain_id` 已从可选升级为强制步骤                                                         |
| **M7**  |  ✅  | 审计并更新所有 writable agent configs：Architect, Coder-BE, Coder-FE, CI-CD-Agent, Meta-Planner 均已注册 `knowledge_cache_attest` |
| **M12** |  ✅  | `project.config.json` knowledge_semantic_map.state_management.save_path 已从 "opencode/framework/" 改为 "opencode/state/"         |
| **M13** |  ✅  | `framework-self-test.ts` 已新增 save_path 唯一性检查（相同/前缀检测）                                                             |

### P2 — 自动化质量门禁（全部完成）

|    #    | 状态 | 实施内容                                                                                                        |
| :-----: | :--: | --------------------------------------------------------------------------------------------------------------- |
| **M8**  |  ✅  | `framework-self-test.ts` 已新增 writable agent knowledge_cache_attest 注册检查                                  |
| **M9**  |  ✅  | `knowledge_cache_attest` 已追加 cache_sufficient / insufficiency_reason 字段，M3/M11 天然阻断 insufficient 状态 |
| **M10** |  ✅  | 已新建 `.opencode/lib/critical-files.ts`，实现 `getCriticalFilesForDomain()`                                    |
| **M14** |  ✅  | `dispatch_subagent` 权限已扩展，子 agent 可 target `@Knowledge-Curator` 直接派遣                                |

### 集成验证状态

- **INT-1** (Complete Bypass Defense Chain): ✅ 全部绕过路径已被 M2+M3+M5+M11 联合拦截
- **INT-2** (M9 Self-Healing Closed Loop): ✅ cache_sufficient=false → KC dispatch → re-attest 闭环已打通
- **INT-3** (Enforcement Mode Matrix): ✅ advisory/strict/locked 三种模式行为差异已在各 Path 中实现
- **INT-4** (Regression — Normal Task Flow): ✅ 各 agent 正常 UC7KS 流程不受影响
