...
**Status**: reviewed — see [Changelog](#changelog) for corrections
...
// On success:
writeLog(SRC, "INFO", { event: "CONFIG-READ-ATTEST", session_id, status: "passed" });

// On failure:
writeLog(SRC, "WARN", { event: "CONFIG-READ-ATTEST", session_id, status: "failed", unread_files });

// On error:
writeLog(SRC, "ERROR", { event: "CONFIG-READ-ATTEST-FAILED", detail: err.message });
```
