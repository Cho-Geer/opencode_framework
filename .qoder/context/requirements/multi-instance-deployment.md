...
Run multiple independent booking system instances simultaneously on a single PC, each with its own database, cache, backend API, and frontend dev server, without mutual interference.

## 2. Functional Requirements

### 2.1 Instance Isolation
- Each instance has an independent PostgreSQL database (different database name, different host port)
- Each instance has an independent Redis cache (different host port)
- Each instance has an independent backend API service (different port)
- Each instance has an independent frontend Angular dev server (different port)
- Each instance has independent Docker container names, volume names, and network names

### 2.2 One-Click Generation and Startup
- Generate instance configuration files via the script `scripts/start-instance.sh <instance-id>`
- Start via `docker compose -f instances/<instance-id>/docker-compose.yml up -d`
- Start the frontend via `npx ng serve --port=<port> --proxyConfig=<path>`

### 2.3 Cleanup and Shutdown
- Support stopping and cleaning up a single instance without affecting other instances
...
- Backend injects configuration via `backend.env` environment variable file
