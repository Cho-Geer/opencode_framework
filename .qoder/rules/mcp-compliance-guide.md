...
## MCP调用自检清单
- [ ] 已查阅`.qoder/rules/rule_detail/mcp-tool-inventory.md`中的"已注册MCP工具清单"
- [ ] 已识别任务类型和关键词，匹配触发关键词
...
- 调用工具：验证符合`.qoder/rules/rule_detail/mcp-tool-inventory.md`中的流程要求

## MCP调用违规预防模式
1. 先规划后执行：任何任务开始前必须先展示MCP工具调用计划
2. 查阅`.qoder/rules/rule_detail/mcp-tool-inventory.md`优先：这是所有任务的第一步
3. 工具必须注册：所有MCP工具必须在`.qoder/rules/rule_detail/mcp-tool-inventory.md`中注册  
