...
**Applies To**: Agent configs (`.qoder/agents/*.md`), rule files (`.qoder/rules/**/*.md`), skill files (`.qoder/skills/**/SKILL.md`), preamble (`.qoder/subagent-preamble.md`), and any document injected into agent prompts by the dispatch system.
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| **Portability**            | The same `.qoder/` framework directory can be copied to any project and work after updating only `project.config.json`                    |
...
| 4   | `{project.contract_hash_command}` | Command to compute keystone hash | `template_resolution.contract_hash_command` | `node .qoder/scripts/mcp-tools/keystone-validate.js --hash contract.yaml` |
...
Used in: `.qoder/rules/backend-coding-standard.md`
...
Used in: `.qoder/rules/frontend-coding-standard.md`
...
Template resolution is performed by `.qoder/scripts/command-tools/dispatch-subagent.js` in the prompt assembly phase (between agent config parsing and final prompt generation).

**Source file**: `.qoder/scripts/command-tools/dispatch-subagent.js`  
**Key functions**:
...
```
┌─────────────────────────────────────────────────┐
│ 1. Read .qoder/project.config.json            │
├─────────────────────────────────────────────────┤
...
3. `framework-self-test.js` Check 17 scans all `.qoder/` files for `UNRESOLVED{` strings and fails if any are found
...
| ----------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| **Agent Configs** | `.qoder/agents/architect.md`, `coder-be.md`, `coder-fe.md`               | At dispatch, before prompt assembly                                                                                                |
| **Rule Files**    | `.qoder/rules/backend-coding-standard.md`, `frontend-coding-standard.md` | These are injected into prompts; extended placeholders remain as `UNRESOLVED{...}` and are interpreted via internal mapping tables |
| **Skill Files**   | `.qoder/skills/*/SKILL.md`                                               | At dispatch, when skill content is injected                                                                                        |
| **Preamble**      | `.qoder/subagent-preamble.md`                                            | At dispatch, line 299-304 of `dispatch-subagent.js` resolves preamble placeholders                                                 |
...
```
.qoder/agents/<agent_type>.md         ← Primary target
.qoder/subagent-preamble.md           ← Resolved before injection
.qoder/rules/backend-coding-standard.md ← Injected as-is (extended placeholders)
.qoder/rules/frontend-coding-standard.md
```
...
1. **Contract file change check**: If the current change involves
   `contract.yaml` or requirement documents:
   - Run `node .qoder/scripts/mcp-tools/keystone-validate.js --hash contract.yaml` to automatically compute
     and update the hash value.
```
...
| **2** | Check that the corresponding key exists in `project.config.json`                                                                            | `.qoder/project.config.json` → `template_resolution` or `tech_stack` |
...
| **5** | Run `framework-self-test.js` to verify resolution                                                                                           | `node .qoder/scripts/framework-self-test.js`                         |
...
4. Search for all usages with: grep -r "{template_key}" .qoder/
...
node .qoder/scripts/command-tools/dispatch-subagent.js Architect "test" 2>&1 | grep UNRESOLVED

# Scan all .qoder files for placeholder patterns
grep -r '\{[a-z_]*\.[a-z_.]*\}' .qoder/ --include="*.md" | grep -v 'UNRESOLVED\|template-variable-standard\|TEMPLATE_VARIABLE_STANDARD\|node_modules'
```
...
| ------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| `.qoder/project.config.json`                        | **Source of truth** for all placeholder values (`template_resolution` + `tech_stack` sections)                  |
| `.qoder/scripts/command-tools/dispatch-subagent.js` | Resolver implementation (`buildTemplateResolutionMap` + `resolveTemplateVariables`)                             |
| `.qoder/scripts/framework-self-test.js`             | Check 17 verifies no orphaned `UNRESOLVED{...}` strings; Check 18 verifies `template_resolution` section exists |
| `.qoder/subagent-preamble.md`                       | Consumer of template variables; all `{template_key}` references resolved before prompt injection                |
| `.qoder/rules/backend-coding-standard.md`           | Consumer of 7 extended `{backend.*}` placeholders (§3.2); contains internal resolution table                    |
| `.qoder/rules/frontend-coding-standard.md`          | Consumer of 5 extended `{frontend.*}` placeholders (§3.3); contains internal resolution table                   |
| `.qoder/agents/architect.md`                        | Consumer of `{project.contract_hash_command}`                                                                   |
| `.qoder/agents/coder-be.md`                         | Consumer of `{backend.orm.schema}` and `{project.contract_hash_command}`                                        |
| `.qoder/agents/coder-fe.md`                         | Consumer of `{frontend.dto_path}`, `{frontend.env_path}`, and `{project.contract_hash_command}`                 |
