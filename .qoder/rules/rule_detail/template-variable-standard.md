---
type: model_decision
description: When processing template variables
---
...
| `{project.contract_hash_command}` | Command to compute keystone hash | `template_resolution.contract_hash_command` | `node .qoder/scripts/mcp-tools/keystone-validate.js --hash contract.yaml` |
...
Template resolution is performed by `.qoder/scripts/command-tools/dispatch-subagent.js` in the prompt assembly phase (between agent config parsing and prompt generation).
...
### §4.1 Agent Configs (`.qoder/agents/*.md`)
...
### §4.3 Skill Files (`.qoder/skills/**/SKILL.md`)
...
### §4.4 Preamble (`.qoder/subagent-preamble.md`)
...
- **Updating resolver logic**: Modify `.qoder/scripts/command-tools/dispatch-subagent.js` → run `framework-self-test.js` Check 18 to verify `template_resolution` is valid.

---

## §8 Related Documents

- `.qoder/project.config.json` — `template_resolution` section (source of truth)
- `.qoder/scripts/command-tools/dispatch-subagent.js` — template resolver implementation
- `.qoder/scripts/framework-self-test.js` — Check 17/18 for placeholder validation
- `.qoder/subagent-preamble.md` — consumer of template variables
