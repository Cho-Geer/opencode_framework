...
- Before reading any `.opencode/context/requirements/` doc, call `search_memory` first
...
- After every Coding task that modifies files matching `qpv-config.json → source_paths`, @Orchestrator MUST dispatch QPV
- **Frontend changes**: Browser subagent verifies affected pages directly
- **Backend changes**: Leader first runs Affected Page Resolution (APR) via `qpv-config.json → endpoint_page_map` to identify consuming pages, then dispatches Browser subagent
...
- All design tokens, thresholds, breakpoints, and error codes are read from `qpv-config.json` — not hardcoded in the skill or standard
