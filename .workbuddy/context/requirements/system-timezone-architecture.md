# Booking System — Timezone-Adaptive Architecture Design Document
...
- **Author**: @Architect
- **Related Documents**: System Architecture Design (SAD).md, Data Architecture Design.md, API Interface Specification.md, contract.yaml
...
| **P2** | **Never trust client timestamps** | Frontend-sent timestamps are treated as "local time with offset"; the backend must re-interpret them based on known timezone context (`X-Timezone` header, user preference, environment default) and convert to UTC. |
| **P3** | **Always annotate with timezone** | Any API response containing date/time must carry sufficient timezone context (IANA timezone field in response, ISO 8601 offset, or global `X-Timezone` header), so the client never needs to guess. |
| **P4** | **Business logic operates in clinic timezone** | Business hours calculation, date boundary judgment ("today", "this month"), statistical aggregation and other business logic uniformly execute in "Clinic Timezone", not UTC or server local time. |
| **P5** | **IANA timezone strings only** | All timezone representations must use IANA timezone database identifiers (e.g., `Asia/Shanghai`, `America/New_York`); UTC offsets (e.g., `+08:00`) are prohibited as business logic input. |
| **P6** | **date-fns-tz for all conversions** | Backend uses `date-fns-tz` (based on `Intl`), frontend reuses existing `date-fns@^3.6.0` + adds `date-fns-tz`, unifying timezone conversion implementation and eliminating implicit `new Date()` behavior differences. |

## 2. Timezone Detection Strategy

A three-tier cascade detection mechanism, tried in order from highest to lowest priority:
...
- Frontend `TimezoneService` obtains browser timezone via `Intl.DateTimeFormat().resolvedOptions().timeZone`
- HTTP interceptor automatically injects `X-Timezone` request header into every API request
- Backend `TimezoneInterceptor` reads this header and attaches it to `req.timezone`
- **Format**: IANA timezone string (e.g., `Asia/Shanghai`)

### 2.2 Tier 2 — User preferredTimezone (Stored Preference)

- At first login, frontend automatically detects browser timezone and submits it to backend for storage
- Users can manually change `preferredTimezone` in personal settings page
- Stored in User model `preferredTimezone` field (varchar(50))
- Updated during login flow (if frontend submitted `X-Timezone` and it differs from database)
...
- This value is also stored in `SystemSetting` table, key = `default_timezone`
- Allows operations staff to modify at runtime without service restart
...
Leverages already-installed `date-fns@^3.6.0` + new `date-fns-tz` dependency, providing unified date formatting service:
...
**Current state**: About 15+ `.html` files use Angular native `date` pipe without timezone parameter, causing browser-implementation-dependent behavior.
...
When frontend creates an appointment:
...
Backend uses this + `X-Timezone` header for final UTC conversion, double verification.

## 4. Backend Architecture

### 4.1 Prisma Schema — User Model Extension

Add `preferredTimezone` field to `User` model:
...
**Constraints**:
- Length limit: `varchar(50)` (longest IANA identifier like `America/Argentina/ComodRivadavia` is ~35 characters)
- Nullable: `null` = use environment default timezone
- Validation: Application layer uses `Intl.supportedValuesOf('timeZone')` to validate legality
...
export class TimezoneInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    if (headerTimezone && this.VALID_TZS.has(headerTimezone)) {
      ...
      req.timezone = null; // Trigger Tier 2/3 fallback
    }
...
Or inject via `APP_INTERCEPTOR` provider pattern.
...
Using existing `SystemSetting` model, add new keys:
...
**Problem (L212, L214)**: Hardcoded `09:00-17:00 UTC`
...
**Problem (L59-62)**: `toISOString().slice(0,10)` uses UTC date boundary
...
**Problem (L154)**: Mixed `setHours()` and `setUTCHours()`

**Refactoring plan**: Unify using `date-fns-tz`'s `toZonedTime` and `formatInTimeZone`, eliminating direct dependency on `setHours`/`setUTCHours`.

#### 4.5.4 appointments.service.ts

**Problem (L198)**: `new Date(string)` parsing depends on string format
...
For all DTOs involving `DateTime`, add decorator validation ensuring timestamps contain timezone information:
...
Add to `contract.yaml` `api.endpoints` documentation:
...
All responses containing `appointmentDate`, `startTime`, `endTime` and other `DateTime` fields use ISO 8601 with offset:
...
During backend serialization, convert based on `ClinicTimezoneProvider.resolve()` result:
```typescript
...
Using existing `SystemSetting` table to store business hours configuration:
...
| `settingType` | `JSON` |
| `category` | `BUSINESS` |
| `description` | `Business hours configuration, defining multiple time ranges per day of week` |
...
interface BusinessHoursConfig {
  ...
  monday: TimeRange[];        // Monday business time ranges
  tuesday: TimeRange[];
  wednesday: TimeRange[];
  thursday: TimeRange[];
  friday: TimeRange[];
...
Generated migration SQL summary:
```sql
...
### 7.2 Data Not Requiring Migration

| Data Table | Field | Notes |
|------------|-------|-------|
| `User` | `createdAt` / `updatedAt` | Already `timestamptz`, UTC storage unchanged |
| `Appointment` | `appointmentDate` | Already `timestamptz`, UTC storage unchanged. Frontend display converts using clinic tz. |
| `TimeSlot` | `startTime` / `endTime` | Already `timestamptz`. Generation logic changed to derive from business hours. |
| `Notification` | `createdAt` / `sentAt` / `scheduledAt` | UTC storage, display conversion. |
...
- Legacy `new Date('YYYY-MM-DD')` calls naturally eliminated after migrating to `date-fns-tz`
...
|-----------|------------|-------|
| `booking-backend/prisma/schema.prisma` | **Modify** | User model add `preferredTimezone String?` |
...
| `booking-backend/src/common/interceptors/timezone-serialization.interceptor.ts` | **New** | DateTime fields in response converted by clinic tz |
...
| `booking-backend/src/modules/admin/appointments/admin-appointments.service.ts` | **Refactor** | L154 `setHours`/`setUTCHours` unification |
...
| `booking-backend/test/**/*.spec.ts` | **Modify** | Adapt to new timezone parameters (~20+ files) |
...
| `booking-frontend/src/app/core/services/date-format.service.ts` | **New** | Wrap `date-fns-tz` formatting API |
| `booking-frontend/src/app/app.config.ts` | **Modify** | Register TimezoneInterceptor |
| `booking-frontend/src/app/features/booking/**/*.ts` | **Modify** | Component inject DateFormatService |
| `booking-frontend/src/app/features/admin/**/*.ts` | **Modify** | Component inject DateFormatService |
| `booking-frontend/src/app/features/**/*.html` (~15+ files) | **Modify** | `date` pipe → `dateFormatService` method |
...
| `booking-frontend/src/test/factories/time-slot.factory.ts` | **Modify** | Factory method timezone adaptation |
...
| `booking-deploy/env/backend.env` | **Modify** | Deploy environment add `DEFAULT_TIMEZONE` |
| `.opencode/context/requirements/system-timezone-architecture.md` | **New** | This document |
| `contract.yaml` | **Modify** | Add X-Timezone header, timezone query param, User.preferredTimezone |
| `.opencode/context/requirements/Data Architecture Design.md` | **Modify** | L104 fix `appointmentDate` type description |
| `.opencode/context/requirements/System Architecture Design (SAD).md` | **Modify** | Add timezone architecture overview section reference |

## 9. Risks and Mitigations

| # | Risk | Impact | Probability | Mitigation |
|---|------|--------|------------|------------|
| **R1** | **Legacy clients without X-Timezone** | New backend cannot receive timezone info, uses default | High (migration period) | Tier 3 env default + Tier 2 user preference as fallback. Legacy client behavior unchanged (displays in UTC). |
| **R2** | **Daylight Saving Time transitions** | Appointment time off by 1 hour around DST transition | Medium | IANA timezone identifiers handle DST automatically; `date-fns-tz` uses `Intl` respecting DST; non-critical times (midnight) unaffected. |
...
| **R4** | **Concurrent timezone config change** | Business hours modification affects ongoing operations | Low | Business hours only affect future slot generation; existing appointments retain their UTC timestamps. Config changes use database transactions + version number optimistic locking. |
| **R5** | **date-fns-tz bundle size** | Frontend adds ~5-8KB gzipped | Medium | Confirmed in bundle analysis, incremental size is acceptable. Can optimize via tree-shaking. |
| **R6** | **Performance overhead** | Every API call needs timezone resolution | Low | `ClinicTimezoneProvider.resolveSync()` is O(1) set lookup; `formatInTimeZone` performance comparable to `Intl.DateTimeFormat`. No additional caching needed. |
| **R7** | **Mutation testing with timezone dependency** | Timezone dependency in tests causes non-deterministic results | Medium | Tests fix `DEFAULT_TIMEZONE=UTC`; use test doubles (spy/stub) to control `ClinicTimezoneProvider` return values. |
...
```
Priority: P0
Goal: Establish timezone infrastructure, no change to existing business behavior
```

| Step | Task | Output |
|------|------|--------|
| 1.1 | Backend `.env` add `DEFAULT_TIMEZONE=Asia/Shanghai` | Environment variable effective |
...
| 2.4 | Refactor `admin-stats.service.ts` | Date boundary calculated in clinic tz |
...
| 2.7 | Create `TimezoneSerializationInterceptor` | Response JSON dates output by clinic tz |
...
| 3.4 | Create `DateFormatService` | Wrap date-fns-tz |
| 3.5 | Register interceptor in `app.config.ts` | Global header injection |
| 3.6 | Replace `date` pipe in 15+ HTML files (batch by module) | No `date` pipe remaining |
...
```
Priority: P1
Goal: Update contract.yaml + improve tests
```
...
| 4.9 | Update date expectation values in frontend component specs | No hardcoded UTC assertions |
...
| Step | Task | Output |
|------|------|--------|
| 5.1 | grep search for `09:00`, `17:00`, `setHours`, `setUTCHours`, `toISOString().slice(0,10)` | Confirm no residuals |
| 5.2 | `npm run test` + `npm run test:cov` full run | Tests pass + coverage ≥70% |
| 5.3 | Update `.opencode/context/requirements/Data Architecture Design.md` L104 | Fix `appointmentDate` type description |
| 5.4 | Update `.opencode/context/requirements/System Architecture Design (SAD).md` | Add timezone architecture overview section reference |
...
{
  "dependencies": {
    "date-fns": "^4.1.0",      // Upgrade (current may be v3)
